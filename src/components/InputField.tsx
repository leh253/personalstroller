...
const InputField: React.FC<InputFieldProps> = ({ icon: Icon, className = '', ...props }) => {
  return (
    <div className={`relative w-full mb-5 group ${className}`}>
      ...
      <input 
        {...props} 
        className={`w-full glass-input py-4 ${Icon ? 'pl-12' : 'pl-4'} pr-4 rounded-2xl outline-none focus:border-gold-400/50 focus:bg-white/10 transition-all placeholder:text-gray-500/80 text-white shadow-inner bg-white/5 border border-white/10`} 
      />
    </div>
  );
};
