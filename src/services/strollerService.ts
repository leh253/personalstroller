...
export const saveQuizHistory = async (answers: QuizAnswers, resultCount: number) => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
    
    // On insère même si l'utilisateur n'est pas connecté (user_id sera null)
    // Assurez-vous que votre table Supabase autorise les user_id NULL ou que vos règles RLS permettent l'insertion anonyme
