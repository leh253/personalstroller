...
import { RefreshCw, ExternalLink, ArrowLeft, Feather } from 'lucide-react';
...
const ResultsScreen: React.FC<Props> = ({ results, onRestart, onBack }) => {
  return (
    <div className="h-full flex flex-col w-full animate-in bg-[#151b2b] relative overflow-hidden">
      
      {/* Background Feathers - Subtle Decoration */}
      <div className="absolute top-[10%] right-[-5%] opacity-[0.03] pointer-events-none rotate-[-15deg]">
        <Feather size={300} strokeWidth={0.5} />
      </div>
      <div className="absolute bottom-[-10%] left-[-10%] opacity-[0.03] pointer-events-none rotate-[30deg]">
        <Feather size={400} strokeWidth={0.5} />
      </div>

      {/* Navbar - Sticky */}
      <div className="sticky top-0 z-30 p-4 md:p-6 bg-[#151b2b]/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between shrink-0">
        <button onClick={onBack} className="p-2 hover:bg-white/5 rounded-full transition-colors text-slate-400 hover:text-white">
          <ArrowLeft size={24} />
        </button>
        <Logo size="small" />
        <div className="w-10"></div> {/* Spacer pour l'équilibre visuel */}
      </div>
        ...
        <div className="max-w-6xl mx-auto pb-24">
          <header className="mb-10 text-center">
            <div className="inline-flex items-center justify-center gap-2 mb-2">
              <Feather className="text-gold-400 w-5 h-5 opacity-60" />
              <h2 className="text-3xl font-bold text-white">Votre Sélection Idéale</h2>
              <Feather className="text-gold-400 w-5 h-5 opacity-60 scale-x-[-1]" />
            </div>
          ...
          {results.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {results.map((stroller, idx) => (
                <div key={idx} className="glass-card rounded-[2rem] overflow-hidden flex flex-col group border border-white/10 bg-white/5 backdrop-blur-sm">
